...
export default function AdviserPage() {
    ...
    useEffect(()=>{
        ...
        axios.get("http://localhost:5000/search2/", {
            headers: {
        ...
        {infos.filter((dt)=>dt._id===myID3).map(infos =>  
            <div key={infos._id} className="adviserContainer">          
                ...
                <div className="divv">
                <h4 className="colorful">Adviser İnformation</h4>
                {infos.name} {infos.surname}
                </div>
                <div className="divv">
                <h4 className="colorful">About Adviser</h4>
                {infos.information}
                </div>
                <div className="divv">
                <h4 className="colorful">Communication Adviser</h4>
                {infos.contact}
                </div>
